...
import { Check, X, Clock, Trophy, Crown, Mic, MicOff, UserMinus } from 'lucide-react';
import { audio } from '../lib/audio';
import { supabaseService } from '../services/supabaseService';
import { storage } from '../lib/storage';
import { MatchEndScreen } from '../components/MatchEndScreen';

export default function FamousRoom() {
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="grid md:grid-cols-4 gap-8">
        <div className="md:col-span-3">
          {currentQuestion && currentQuestion.famousData && (
            <div className="bg-slate-800 p-6 md:p-12 rounded-3xl border border-slate-700 space-y-6 md:space-y-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {['A', 'B'].map((opt) => {
                  const famousOption = opt === 'A' ? currentQuestion.famousData.optionA : currentQuestion.famousData.optionB;
                  ...
                  return (
                    <button
                      ...
                      className={`relative p-8 rounded-3xl border-4 transition-all font-bold flex flex-col items-center justify-center gap-4 ${btnColor} ${me?.hasAnsweredCurrentRound && !isSelected && !isRevealing ? 'opacity-50' : ''}`}
                    >
                      <div className="text-6xl">{famousOption.emoji}</div>
                      <div className="text-xl font-bold">{famousOption.name}</div>
                      {isRevealing && (
                         <div className={`text-2xl font-mono mt-2 ${isCorrect ? 'text-emerald-400' : 'text-red-400'}`}>
                           {famousOption.value} {currentQuestion.famousData.unit}
                         </div>
                      )}
                      {isRevealing && isCorrect && <Check className="absolute top-4 right-4 w-6 h-6" />}
                      {isRevealing && isSelected && !isCorrect && <X className="absolute top-4 right-4 w-6 h-6" />}
                    </button>
                  );
                })}
              </div>
