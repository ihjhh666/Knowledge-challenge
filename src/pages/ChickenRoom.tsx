...
export default function ChickenRoom() {
  const { state, playerId, isHost, sendChickenEvent, returnToLobby, requestRematch } = useGame();
  ...
  const update = (dt: number) => {
    ...
    if (localPlayer) {
        ...
        if (!isHost && frameCountRef.current % 2 === 0) {
            sendChickenEvent({ type: 'CHICKEN_INPUT', playerId, dx, dy, sprintIntent });
        }
    }
    ...
    let targetCamX = targetP ? (targetP.x - viewW / 2) : 0;
    let targetCamY = targetP ? (targetP.y - viewH / 2) : 0;
  ...
  const checkWinCondition = useCallback(() => {
      if (!isHost) return;
      let winningPlayer = null; // actually find first player to 50
  ...
  }, [gameState, endGame, executeRandomEvent, isHost]);
  ...
  const draw = () => {
    ...
    renderList.forEach(item => {
       if (item.type === 'pond') {
           ...
           ctx.fillStyle = p.id === playerId ? '#38bdf8' : 'white'; ctx.fillText(p.name, 0, 1);
           if (p.id === playerId) { ctx.fillStyle = 'rgba(56, 189, 248, 0.5)'; ctx.beginPath(); ctx.moveTo(-6, 15); ctx.lineTo(6, 15); ctx.lineTo(0, 22); ctx.fill(); }
           ctx.restore();
       }
    });
    ...
    playersRef.current.forEach(p => {
        ...
        if (p.id === playerId) { ctx.shadowColor = 'white'; ctx.shadowBlur = 4; ctx.stroke(); ctx.shadowBlur = 0; }
    });
    ctx.fillStyle = 'white';
    chickensRef.current.forEach(c => {
  ...
  return (
    <div className="fixed inset-0 z-[100] w-full h-[100svh] bg-slate-950 flex flex-col touch-none overflow-hidden font-sans">
      ...
      >
        ...
        <div className="absolute inset-0 z-20 pointer-events-none overflow-hidden">
          ...
          <div className="absolute top-4 left-4 right-4 flex justify-between items-start">
            <div className="flex gap-2 pointer-events-auto">
               <button onClick={returnToLobby} className="w-10 h-10 flex items-center justify-center bg-black/30 backdrop-blur-md text-white rounded-xl hover:bg-black/50 transition-all shadow-lg border border-white/10 active:scale-95">
                   ...
                   {scores.slice(0, 2).map((s) => (
                      <div key={s.id} className="flex items-center justify-between min-w-[70px]">
                         <div className="flex items-center gap-1.5">
                             ...
                             <span className={`text-xs font-bold ${s.id === playerId ? 'text-white' : 'text-slate-300'}`}>{s.name.substring(0, 8)}</span>
                         </div>
        ...
        {gameState === 'playing' && (
          <div 
            className="absolute bottom-6 left-6 w-16 h-16 z-30 touch-none pointer-events-auto rounded-full bg-black/10 border-2 border-white/10 backdrop-blur-[2px] shadow-lg flex items-center justify-center p-0 m-0 opacity-40 active:opacity-100 transition-opacity"
          ...
          >
             <div 
                ...
                style={{ width: 24, height: 24, transform: 'translate(0px, 0px)' }} 
             />
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xl animate-in fade-in duration-300">
           <div className="bg-slate-900/90 p-8 rounded-[2rem] shadow-2xl max-w-md w-full border border-slate-800 text-center animate-in zoom-in-95 duration-500">
              ...
              <p className="text-slate-400 text-lg mb-8 font-medium">
                 {isWin ? 'لقد جمعت الدجاج أسرع من الجميع.' : 'اللاعبون الآخرون كانوا أسرع منك هذه المرة.'}
              </p>
              ...
              <div className="grid grid-cols-2 gap-4">
                 <button onClick={requestRematch} className="bg-emerald-500 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 hover:bg-emerald-600 transition-colors shadow-lg active:scale-95">
                    <RotateCcw className="w-6 h-6" /> العب مجدداً
                 </button>
                 <button onClick={returnToLobby} className="bg-slate-800 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 hover:bg-slate-700 transition-colors shadow-lg border border-slate-700 active:scale-95">
