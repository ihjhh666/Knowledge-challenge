...
export default function HockeySolo() {
      ...
      if (distSq <= minRadius*minRadius) {
        ...
        for (let i = 0; i < 5 + Math.min(10, intensity); i++) {
            particlesRef.current.push({
                ...
                life: 1,
                color: paddle === playerPaddleRef.current ? '#34d399' : '#fb7185',
  ...
  const draw = (ctx: CanvasRenderingContext2D) => {
    ...
    drawPaddle(botPaddleRef.current.pos.x, botPaddleRef.current.pos.y, botPaddleRef.current.radius, '#fb7185', '#881337', '#f43f5e', false);
    drawPaddle(playerPaddleRef.current.pos.x, playerPaddleRef.current.pos.y, playerPaddleRef.current.radius, '#34d399', '#064e3b', '#10b981', true);
