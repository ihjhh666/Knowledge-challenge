...
export default function IceRoom() {
  const navigate = useNavigate();
  const { state, playerId, isHost, sendIceEvent, returnToLobby, requestRematch } = useGame();
  ...
  const remoteInputs = useRef<{ [id: string]: { dx: number, dy: number } }>({});
  ...
  useEffect(() => {
    if (gameState === 'gameover' && !statsSaved && playerPlacement > 0) {
        setIsWin(playerPlacement === 1);
        const pName = storage.getPlayerName() || 'أنت';
        supabaseService.updatePlayerStats(storage.getPlayerId(), pName, playerPlacement === 1, 0, 0, myScoreRef.current.score, '🧊 الجليد المنزلق');
        setStatsSaved(true);
    }
  }, [gameState, statsSaved, playerPlacement]);
  ...
  const botColors = [
      ...
      { body: '#047857', belly: '#d1fae5' }, { body: '#d97706', belly: '#fef3c7' }
  ];
  ...
  const initGame = useCallback(() => {
    ...
    myScoreRef.current = { score: 0, time: 0 };
  ...
  useEffect(() => {
    ...
    let chars = charsRef.current;
    if (chars.length === 0) {
        // First initialization
        const newChars: Character[] = [];
        roomPlayers.forEach((p, idx) => {
            const rx = GRID_W / 2 + (Math.random() - 0.5) * 400;
            const ry = GRID_H / 2 + (Math.random() - 0.5) * 400;
            newChars.push({
               id: p.id, name: p.username, isBot: false,
               x: rx, y: ry, vx: 0, vy: 0, radius: 12, falling: false, fallScale: 1, angle: 0, dead: false,
               colorBody: botColors[idx % 6].body, colorBelly: botColors[idx % 6].belly, hatType: idx % 2,
               ix: 0, iy: 0, aiChangeTimer: 0, score: 0, timeSurvived: 0
            });
        });
        
        if (isHost && newChars.length < 6) {
           const botNames = ['Frost', 'Ice', 'Snow', 'Polar', 'Freeze', 'Glacier'];
           while(newChars.length < 6) {
               const idx = newChars.length;
               newChars.push({
                   id: `bot_${idx}`, name: botNames[idx-1], isBot: true,
                   ...
                   colorBody: botColors[idx % 6].body, colorBelly: botColors[idx % 6].belly, hatType: idx % 2,
                   ix: 0, iy: 0, aiChangeTimer: Math.random(), score: 0, timeSurvived: 0
               });
           }
        }
        ...
        // Just update names/presence if needed, but in reality we shouldn't change chars mid-game 
        // to avoid snapping them.
    }
  }, [state?.players, isHost]);

  useEffect(() => {
     const handleIceEvent = (e: any) => {
         const msg = e.detail;
         if (msg.type === 'ICE_INPUT' && isHost) {
             remoteInputs.current[msg.playerId] = { dx: msg.dx, dy: msg.dy };
         } else if (msg.type === 'ICE_SYNC' && !isHost) {
             if (msg.tiles && tilesRef.current.length === 0) {
                 tilesRef.current = msg.tiles;
             } else if (msg.tiles) {
                 // Fast sync tiles
                 msg.tiles.forEach((mt: any, i: number) => {
                     if (tilesRef.current[i]) {
                         if (tilesRef.current[i].state !== mt.state) {
                             if (mt.state === 2) {
                                 createSnow(mt.c * TILE_SIZE + TILE_SIZE/2, mt.r * TILE_SIZE + TILE_SIZE/2, 24);
                                 audio.click();
                             }
                         }
                         tilesRef.current[i].state = mt.state;
                         tilesRef.current[i].crackTimer = mt.crackTimer;
                     }
                 });
             }
             ...
             if (msg.chars) {
                 if (charsRef.current.length === 0) {
                     ...
                     msg.chars.forEach((mc: any) => {
                         ...
                         if (localP) {
                             localP.dead = mc.dead;
                             localP.falling = mc.falling;
                             localP.fallScale = mc.fallScale;
                             localP.score = mc.score;
                             
                             if (localP.id !== playerId) {
                                 localP.targetX = mc.x; localP.targetY = mc.y;
                                 localP.targetVx = mc.vx; localP.targetVy = mc.vy;
                                 localP.angle = mc.angle;
                             } else {
                                 if (Math.hypot(localP.x - mc.x, localP.y - mc.y) > 80) {
                                     localP.x = mc.x; localP.y = mc.y;
                                 }
                             }
                         }
                     });
             ...
             if (msg.state) {
                 ...
                 if (msg.playerPlacement) {
                     setPlayerPlacement(msg.playerPlacement[playerId] || 0);
                 }
             }
         }
     };
     window.addEventListener('ice_event', handleIceEvent);
     return () => window.removeEventListener('ice_event', handleIceEvent);
  }, [isHost, playerId]);
  ...
  const update = useCallback((dt: number) => {
    ...
    if (curLevel > level) {
      if (isHost) audio.click();
      setLevel(curLevel);
    }
    
    const chars = charsRef.current;
    
    let localPlayer = chars.find(c => c.id === playerId);
    if (localPlayer && !localPlayer.dead) {
        myScoreRef.current.time = Math.floor(gameTimeRef.current);
        myScoreRef.current.score = myScoreRef.current.time * 10 + (level * 50);
    }
    ...
    if (localPlayer) {
        ...
        if (!isHost && frameCountRef.current % 2 === 0) {
            sendIceEvent({ type: 'ICE_INPUT', playerId, dx: ix, dy: iy });
        }
    }
    ...
    if (!isHost) {
        ...
        chars.forEach(p => {
             if (!p.dead) {
                 currentAlive++;
                 if (p.id !== playerId && p.targetX !== undefined && p.targetY !== undefined) {
                     p.x += (p.targetX - p.x) * dt * 10;
                     p.y += (p.targetY - p.y) * dt * 10;
                     p.vx = p.targetVx || 0;
                     p.vy = p.targetVy || 0;
                 }
             }
             if (p.id === playerId && !p.dead && !p.falling) {
                let ix = inputsRef.current.x;
                let iy = inputsRef.current.y;
                if (joystickRef.current.active) {
                   ix = joystickRef.current.dx;
                ...
                if (bounced && Math.sqrt(p.vx*p.vx + p.vy*p.vy) > 50) audio.tick(); 
             }
        ...
        let playerPlacements: Record<string, number> = {};
        ...
        chars.forEach(p => {
            ...
            if (p.falling) {
                ...
                if (p.fallScale <= 0) {
                    ...
                    playerPlacements[p.id] = currentAlive + 1;
                    
                    if (currentAlive <= 1) {
                        const winner = chars.find(c => !c.dead);
                        if (winner) playerPlacements[winner.id] = 1;

                        setGameState('gameover');
                        stateRef.current = 'gameover';
                        sendIceEvent({ type: 'ICE_SYNC', state: 'gameover', playerPlacement: playerPlacements });
                        setPlayerPlacement(playerPlacements[playerId] || 0);
                    }
            ...
            if (!p.isBot) {
                if (p.id === playerId) {
                    ...
                    ix = remoteInputs.current[p.id].dx;
                    iy = remoteInputs.current[p.id].dy;
                }
            } else {
                p.aiChangeTimer -= dt;
                ...
                if (p.aiChangeTimer <= 0 || dangerAhead) {
                    ...
                    if (dangerAhead || (myTile && myTile.state !== 0) || p.x < 100 || p.x > GRID_W - 100 || p.y < 100 || p.y > GRID_H - 100) {
                        ...
                        const playerAlive = chars.find(c => !c.isBot && !c.dead);
            ...
            if (bounced) {
                ...
                if (Math.sqrt(p.vx*p.vx + p.vy*p.vy) > 50) audio.tick(); 
            }
            ...
            if (!isOverSafeTile && !p.falling) {
              p.falling = true;
              audio.click();
              if (p.id === playerId) shakeRef.current = 15;
        ...
        if (frameCountRef.current % 2 === 0) {
            sendIceEvent({
                ...
                chars: chars.map(c => ({
                    ...
                    dead: c.dead, falling: c.falling, fallScale: c.fallScale,
                    angle: c.angle, score: c.score, timeSurvived: c.timeSurvived
                })),
                tiles: tilesRef.current.map(t => ({ c: t.c, r: t.r, state: t.state, crackTimer: t.crackTimer })),
    ...
    // Camera follow logic
    let playerTarget = charsRef.current.find(c => c.id === playerId);
  ...
  }, [state, isHost, playerId, sendIceEvent, level]);
  ...
  const draw = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    if (gameState === 'playing' || gameState === 'gameover') {
         ...
         ctx.textAlign = 'right';
         ctx.fillText(`🎯 ${myScoreRef.current.score}`, width - 20, 44);
  ...
  return (
    <div className="fixed inset-0 z-[100] w-full h-[100svh] overflow-hidden bg-slate-950 font-sans touch-none" dir="rtl">
        ...
        <AnimatePresence>
            {gameState === 'playing' && charsRef.current.find(c => c.id === playerId)?.dead && (
                 <motion.div 
                    initial={{ opacity: 0, y: -20 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="absolute top-2 left-1/2 -translate-x-1/2 z-10 bg-slate-900/80 text-slate-300 px-6 py-2 rounded-full font-bold shadow-lg backdrop-blur text-sm border border-slate-700"
                 >
                    تم إقصاؤك - وضع المشاهدة
                 </motion.div>
            )}
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-[2rem] p-6 max-w-md w-full shadow-2xl my-auto">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-4">
                            <div className="bg-slate-800/50 rounded-2xl p-4 border border-slate-700/50 text-center">
                                ...
                                <div className="text-2xl font-bold font-mono text-white">{myScoreRef.current.score}</div>
                            </div>
                            <div className="bg-slate-800/50 rounded-2xl p-4 border border-slate-700/50 text-center">
                                <div className="text-sky-400 mb-1"><Star className="w-6 h-6 mx-auto" /></div>
                                <div className="text-sm text-slate-400 mb-1">الصمود</div>
                                <div className="text-2xl font-bold font-mono text-white">{myScoreRef.current.time} ث</div>
                            </div>
                        </div>

                        <div className="bg-slate-800/50 rounded-2xl p-4 mb-8 border border-slate-700/50 space-y-3">
                           {charsRef.current.slice().sort((a,b) => b.score - a.score).map((s, idx) => (
                              <div key={s.id} className={`flex justify-between items-center p-3 rounded-xl ${s.id === playerId ? 'bg-slate-700 border border-slate-600' : 'bg-transparent'}`}>
                                 <div className="flex items-center gap-3">
                                     <span className="font-bold text-xl w-6 text-left" style={{color: s.colorBody}}>#{idx+1}</span>
