...
import { ArrowLeft, Heart, XCircle, CheckCircle, Sparkles, Trophy, Clock } from 'lucide-react';
    ...
    <div className="absolute inset-0 pointer-events-none flex items-center justify-center z-50">
      {[...Array(20)].map((_, i) => (
        <motion.div
           ...
           style={{ backgroundColor: ['#A855F7', '#C084FC', '#E879F9', '#F472B6'][Math.floor(Math.random() * 4)] }}
        />
      ))}
    </div>
  );
...
export default function EmojiGuess() {
  ...
  useEffect(() => {
    setQuestions(generateEmojiOptionQuestions(20, answeredIds));
    const handleAch = () => safePlayObject(tfAudio.achievement);
  ...
  const handleAnswer = (answer: string | null, isTimeOut: boolean = false) => {
    ...
    // Choose selected visually
    if (answer) {
      setSelectedAnswer(answer);
    } else {
      // If timeout, pick a wrong answer visually so it triggers wrong logic
      const wrongOpt = currentQ.options.find(o => o !== currentQ.correct) || currentQ.options[0];
      setSelectedAnswer(wrongOpt);
    }
    ...
    setTimeout(() => {
      ...
      if (hearts > 1 || isCorrect) {
        ...
        if (currentIndex > questions.length - 10) {
          setQuestions(q => [...q, ...generateEmojiOptionQuestions(20, answeredIds)]);
        }
      }
    }, 1500);
  };
  ...
  const getEvaluation = (correct: number) => {
    if (correct >= 30) return { title: 'أسطورة الرموز', color: 'text-purple-400', blur: 'bg-purple-400/20' };
  ...
  const endGame = async () => {
    ...
    const isNewRecord = score > 0 && score > (localStats.emojiHighScore || 0);
    ...
    doUpdateAchStats({
      emojiRoundsPlayed: (localStats.emojiRoundsPlayed || 0) + 1,
      emojiCorrectAnswers: (localStats.emojiCorrectAnswers || 0) + correctCount,
      emojiBestStreak: Math.max(localStats.emojiBestStreak || 0, Math.max(streak, bestStreak)),
      emojiHighScore: Math.max(localStats.emojiHighScore || 0, score),
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white font-sans overflow-hidden">
      ...
      <main className="max-w-4xl mx-auto p-4 md:p-8 flex flex-col items-center justify-center min-h-[calc(100vh-80px)] pb-10 relative">
        <AnimatePresence>
          {isFlashing && (
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute inset-0 bg-rose-500/20 pointer-events-none z-10 shadow-[inset_0_0_100px_rgba(244,63,94,0.5)] border-[8px] border-rose-500"
            />
          )}
        </AnimatePresence>
        ...
        <AnimatePresence mode="wait">
          {!gameOver ? (
            ...
            >
              ...
              <div className="bg-slate-900 border border-slate-700 p-8 md:p-12 rounded-3xl shadow-xl w-full max-w-2xl text-center">
                 <h2 className="text-6xl md:text-8xl tracking-[0.2em] font-extrabold text-white drop-shadow-2xl">
                   {currentQ.emojis}
                 </h2>
              </div>
              ...
              {streak > 1 && (
                <motion.div
                  ...
                  className="bg-amber-500/20 text-amber-400 border border-amber-500/30 px-4 py-1.5 rounded-full text-sm font-bold shadow-[0_0_15px_rgba(245,158,11,0.2)] flex items-center gap-2"
                >
                  <span>🔥</span>
                  متتالية x{streak}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-2xl mt-4 px-2">
                 {currentQ.options.map(opt => (
                   <OptionButton
                     ...
                     isCorrect={currentQ.correct === opt}
             ...
             >
               ...
               <div className="grid grid-cols-2 gap-4 w-full mb-8 relative z-10">
                 <div className="bg-slate-800/80 backdrop-blur-sm p-4 rounded-2xl border border-slate-700/80">
                   <div className="text-2xl font-bold text-purple-400 mb-1">{score}</div>
                 ...
                 <div className="bg-slate-800/80 backdrop-blur-sm p-4 rounded-2xl border border-slate-700/80">
                   ...
                   <div className="text-xs text-slate-400 uppercase tracking-widest font-bold">متتالية</div>
                 </div>
                 <div className="bg-slate-800/80 backdrop-blur-sm p-4 rounded-2xl border border-slate-700/80">
               ...
               <div className="text-center w-full mb-8 bg-purple-500/10 p-4 rounded-2xl border border-purple-500/20 relative z-10">
                 <div className="text-sm text-purple-300 font-bold mb-1 flex items-center justify-center gap-2">
               ...
               <div className="space-y-3 w-full relative z-10">
                 <button 
                   ...
                   className="w-full bg-gradient-to-r from-purple-600 to-purple-800 hover:from-purple-500 hover:to-purple-700 text-white font-bold py-4 rounded-2xl shadow-[0_0_20px_rgba(168,85,247,0.3)] transition-all active:scale-95 text-lg"
                 >
                   العب مرة أخرى
                 </button>
                 <button 
                   onClick={() => navigate('/')}
                   className="w-full bg-slate-800 hover:bg-slate-700 text-white font-bold py-4 rounded-2xl shadow-lg transition-transform active:scale-95 border border-slate-700 text-lg"
                 >
                   العودة للرئيسية
                 </button>
               </div>
...
function OptionButton({ text, isSelected, showAnswer, isCorrect, disabled, onClick }: any) {
  let stateStyles = "bg-slate-800/80 border-slate-700 hover:bg-slate-750 hover:border-slate-500";
  
  if (showAnswer) {
    if (isCorrect) {
      stateStyles = "bg-emerald-500/20 border-emerald-500 shadow-[0_0_30px_rgba(16,185,129,0.3)] saturate-150 z-10 scale-[1.02]";
    } else if (isSelected && !isCorrect) {
      stateStyles = "bg-rose-500/20 border-rose-500 shadow-[0_0_30px_rgba(244,63,94,0.3)] saturate-150";
    } else {
      stateStyles = "bg-slate-800/50 border-slate-800 opacity-40 grayscale";
    }
  } else if (isSelected) {
    stateStyles = "bg-purple-500/20 border-purple-500";
  }
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`relative w-full py-4 px-6 flex items-center justify-center rounded-2xl border-2 transition-all duration-300 group ${stateStyles}`}
    >
      <div className="relative z-10 flex flex-col items-center w-full justify-center">
        <h3 className="text-xl md:text-2xl font-bold font-heading text-white text-center break-words w-full drop-shadow-lg leading-tight">
          {text}
        </h3>
      </div>
      ...
      <AnimatePresence>
        {showAnswer && isCorrect && isSelected && (
          <motion.div
            ...
            className="absolute top-1/2 -translate-y-1/2 right-4 text-emerald-400 bg-emerald-950/50 p-1 rounded-full border border-emerald-500/50 backdrop-blur-sm z-20"
          >
        ...
        {showAnswer && !isCorrect && isSelected && (
          <motion.div
            initial={{ scale: 0, opacity: 0, rotate: 45 }}
            animate={{ scale: [1, 1.5, 1], opacity: 1, rotate: 0 }}
            className="absolute top-1/2 -translate-y-1/2 right-4 text-rose-400 bg-rose-950/50 p-1 rounded-full border border-rose-500/50 backdrop-blur-sm z-20"
          >
