...
import { ArrowRight, Trophy, Crown, RefreshCcw, Settings, Lightbulb, Zap } from 'lucide-react';
...
export default function KingMode() {
  ...
  const nextEventTimer = useRef(Math.floor(Math.random() * 600 + 400)); // 6.6s to 16.6s initially
  ...
  const setupGame = () => {
    ...
    entities.current = [
      { id: 'player', name: 'أنت', isPlayer: true, x: 0, y: 300, vx: 0, vy: 0, radius: 25, color: '#ef4444', speed: 8.5, score: 0, isKing: false, immunityTimer: 0, energy: 100 },
      { id: 'bot1', name: 'بوت 1', isPlayer: false, x: -300, y: -200, vx: 0, vy: 0, radius: 25, color: '#3b82f6', speed: 7.5, score: 0, isKing: false, immunityTimer: 0, energy: 100 },
      { id: 'bot2', name: 'بوت 2', isPlayer: false, x: 300, y: -200, vx: 0, vy: 0, radius: 25, color: '#a855f7', speed: 7.5, score: 0, isKing: false, immunityTimer: 0, energy: 100 },
      { id: 'bot3', name: 'بوت 3', isPlayer: false, x: 0, y: -400, vx: 0, vy: 0, radius: 25, color: '#22c55e', speed: 7.5, score: 0, isKing: false, immunityTimer: 0, energy: 100 },
    ];
  ...
  const updatePhysics = () => {
    ...
    if (activeEvent.current) {
        ...
        if (activeEvent.current.timer <= 0) {
            activeEvent.current = null; // event over
        }
    }

    // Sprint logic
    const playerSprintIntent = keys.current[' '] || isSprintingActive.current || keys.current['shift'];
    ...
    if (player) {
      ...
      const isSprinting = playerSprintIntent && player.energy > 5 && len > 0.1;
      let currentSpeed = player.speed * globalSpeedMultiplier;

      if (isSprinting) {
          player.energy = Math.max(0, player.energy - 1.5);
          currentSpeed *= 1.8;
          if (frameCount.current % 3 === 0) spawnParticles(player.x, player.y + 15, 'rgba(255,255,255,0.5)', 2, 0.2); // dust
      } else {
          player.energy = Math.min(100, player.energy + 1.0);
      }

      if (len > 0) {
        // High acceleration for instant responsiveness
        let nx = dx;
        let ny = dy;
        
        // Normalize keyboard input if diagonal (len > 1) to prevent faster diagonal movement
        if (!joystick.current.active && len > 0.1) {
            nx = dx / len;
            ny = dy / len;
        }
        ...
        player.vy += (targetVy - player.vy) * 0.8;
      } else {
    ...
    if (king) {
        ...
        if (king.score >= WIN_SCORE) {
            setGameState(king.isPlayer ? 'won' : 'lost');
            setWinner(king);
  ...
  return (
    <>
      ...
      <div className="absolute inset-0 flex flex-col font-sans overflow-hidden touch-none overscroll-none z-10" dir="rtl">
        ...
        {/* Modern HUD Overlay */}
        {gameState === 'playing' && (
        <div className="absolute inset-0 pointer-events-none z-10 font-sans flex flex-col justify-between">
            {/* Top Bar Area */}
            <div className="p-4 flex items-start justify-between">
                
                {/* Top Left: Leaderboard & Game Title */}
                <div className="flex flex-col gap-2 items-start w-[140px] sm:w-48 pointer-events-auto">
                    <button 
                        onClick={() => navigate('/')} 
                        className="h-10 px-4 bg-slate-900/80 backdrop-blur-md rounded-full border border-slate-700/50 hover:bg-slate-800 flex items-center gap-2 transition-all shadow-lg text-slate-300"
                    >
                        <ArrowRight className="w-4 h-4" />
                        <span className="text-white font-bold text-xs hidden sm:inline">خروج</span>
                    </button>
                    
                    {/* Compact Leaderboard */}
                    <div className="bg-[#15132b]/80 backdrop-blur-md rounded-2xl border border-[#2d2b45] p-2 shadow-lg w-full">
                        <h3 className="text-slate-400 font-bold text-[10px] uppercase mb-1 px-1 flex items-center justify-between">
                            الترتيب <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse"></span>
                        </h3>
                        <div className="flex flex-col gap-1">
                            {sortedLeaderboard.slice(0, 4).map((e, index) => (
                                <div key={e.id} className={`flex items-center justify-between px-2 py-1 rounded-xl ${e.isPlayer ? 'bg-indigo-500/20 border border-indigo-500/30' : ''}`}>
                                    <div className="flex items-center gap-1.5 overflow-hidden">
                                        <span className="text-white/50 text-[10px] w-3 font-bold">{index+1}</span>
                                        <span className={`text-xs font-bold truncate max-w-[50px] sm:max-w-[70px] ${e.isPlayer ? 'text-indigo-300' : 'text-slate-300'}`}>
                                            {e.name}
                                        </span>
                                    </div>
                                    <div className="flex items-center gap-1 shrink-0">
                                        <span className={`text-xs font-bold ${e.isKing ? 'text-amber-400' : 'text-slate-200'}`}>{Math.floor(e.score)}</span>
                                        {e.isKing && <Crown className="w-3 h-3 text-amber-400" />}
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    {activeEvent.current && (
                        <div className="animate-in fade-in slide-in-from-top-2 duration-300 pointer-events-none w-full">
                            <div className="bg-gradient-to-r from-indigo-600/90 via-purple-600/90 to-indigo-600/90 backdrop-blur-md px-3 py-1.5 rounded-xl border border-purple-400/50 shadow-lg text-center">
                                <span className="text-white font-bold text-[10px] sm:text-xs drop-shadow-md">
                ...
                {/* Top Center: Score & Goal & Timer */}
                <div className="flex flex-col items-center gap-2 mt-0 sm:mt-2 pointer-events-auto">
                    {/* Score Badge */}
                    <div className="flex items-stretch bg-[#15132b]/90 backdrop-blur-md rounded-full border border-[#2d2b45] shadow-2xl overflow-hidden">
                        <div className="px-3 py-1.5 sm:px-5 sm:py-2 flex items-center justify-center gap-1 sm:gap-2 bg-amber-500/10">
                            <Crown className="w-4 h-4 sm:w-5 sm:h-5 text-amber-400 drop-shadow-[0_0_5px_rgba(251,191,36,0.6)]" />
                            <span className="text-2xl sm:text-3xl font-black text-amber-400 font-heading drop-shadow-md">{playerScore}</span>
                        </div>
                        <div className="px-3 py-1.5 sm:px-4 sm:py-2 flex flex-col justify-center items-center bg-[#1e1c32]/50 border-r border-[#2d2b45]">
                            <span className="text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-[-2px]">الهدف</span>
                            <span className="text-sm sm:text-base font-bold text-slate-200">{WIN_SCORE}</span>
                        </div>
                    </div>

                    {/* Timer Badge */}
                    <div className="bg-[#15132b]/80 backdrop-blur-md px-3 py-1 rounded-full border border-[#2d2b45] flex items-center gap-1.5 shadow-lg">
                        <span className="text-[10px] sm:text-xs">⏱️</span>
                        <span className="text-xs sm:text-sm font-bold text-slate-300 font-mono tracking-wide">{formatTime(gameTime)}</span>
                    </div>
                </div>

                {/* Top Right: Settings / Blank */}
                <div className="w-[140px] sm:w-48 flex justify-end">
                    <button className="pointer-events-auto h-10 w-10 bg-[#15132b]/80 backdrop-blur-md border border-[#2d2b45] rounded-full flex items-center justify-center hover:bg-[#2d2b45] transition-colors shadow-lg">
            ...
            <div className="p-6 flex justify-between items-end md:hidden pointer-events-none">
                <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-full border-2 border-white/10 flex items-center justify-center bg-black/20 pointer-events-auto backdrop-blur-[2px]"
                     ref={joystickBgRef}
                ...
                >
                    <div ref={joystickHandleRef} className="w-12 h-12 bg-white/30 backdrop-blur-md shadow-lg border border-white/30 rounded-full transition-transform duration-75" />
                </div>

                <div className="pointer-events-auto">
                    <button 
                         className={`w-14 h-14 sm:w-16 sm:h-16 rounded-full flex items-center justify-center shadow-xl transition-all active:scale-90 ${entities.current.find(e => e.isPlayer)?.energy! > 5 ? 'bg-gradient-to-tr from-amber-600 to-yellow-400 text-white border-2 border-amber-300/50 shadow-[0_0_20px_rgba(245,158,11,0.5)]' : 'bg-[#15132b] text-slate-500 border-2 border-[#2d2b45]'}`}
           ...
           <div className="absolute inset-0 bg-[#0f0e1f]/90 backdrop-blur-xl z-50 flex flex-col items-center justify-center p-6 text-center pointer-events-auto">
              <div className="bg-[#15132b] rounded-[2rem] p-8 max-w-sm w-full border border-[#2d2b45] shadow-[0_0_80px_rgba(0,0,0,0.8)] relative overflow-hidden transform scale-100 transition-all">
                 ...
                 <div className="flex flex-col gap-3">
                     <button 
                        onClick={setupGame}
                        className="w-full bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 focus:ring-4 focus:ring-amber-500/50 text-white font-bold py-4 rounded-2xl transition-all flex items-center justify-center gap-2 outline-none group shadow-lg drop-shadow-md"
                     >
