...
code = code.replace(/const startGame =.*?};/gs, ""); // We will re-implement this carefully. Let's just remove startGame to bot logic.
// actually it's easier to replace it with dummy for now
code = code.replace(/const startGame = \(diff: BotDifficulty\) => \{([^}]+|\{[^}]*\})*\};/g, "");

code = code.replace(/const handleTimeOut = \(\) => \{([^}]+|\{[^}]*\})*\};/g, "");
code = code.replace(/const getBotDecision = \(.*?\): Direction => \{([^}]+|\{[^}]*\})*\};/g, "");
