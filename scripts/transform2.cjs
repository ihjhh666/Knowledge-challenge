...
let code = fs.readFileSync('src/pages/ChickenSolo.tsx', 'utf-8');
...
code = code.replace(
    /const executeRandomEvent = useCallback\(\(\) => \{[\s\S]*?\n  \}, \[\]\);/,
...
code = code.replace(
    /const checkWinCondition = useCallback\(\(\) => \{[\s\S]*?\n  \}, \[\]\);/,
...
code = code.replace(
    /const endGame = useCallback\(\(won: boolean\) => \{[\s\S]*?\n  \}, \[\]\);/,
...
code = code.replace(
    /const update = \(dt: number\) => \{[\s\S]*?const canvas = canvasRef\.current;/,
...
code = code.replaceAll("s.id === 'p1'", "s.id === playerId");
code = code.replaceAll("p.id === 'p1'", "p.id === playerId");

code = code.replace(
    /\{\/\* 7\. Ad Area - Fixed Height \*\/\}[\s\S]*?<\/div>/,
    ""
);
